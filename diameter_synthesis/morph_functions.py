...
def _partition_asymetry_length(section):
    var_n = float(sum([_sec_length(sec) for sec in section.children[0].ipreorder()]))
    var_m = float(sum([_sec_length(sec) for sec in section.children[1].ipreorder()]))
    return abs(var_n - var_m)


def sequential_single(
    seq, neurite=None, section=None
    ...
    if seq == "asymmetry_pair":
        ...
        if section is not None and neurite is None:
            try:
                return [
                    nm.features.bifurcationfunc.partition_pair(section),
                ]
...
def sequential(data, seq, neurite, bounds=None):
    ...
    return [
        [d, s]
        for d, s in zip(data, sequential_single(seq, neurite))
...
def rall_reduction_factor(
    rall_deviation, sibling_ratio, seq=None
):  # noqa, pylint: disable=unused-argument
    """Returns the reduction factor for bifurcation diameter"""
    if sibling_ratio > 0:
        return (1.0 / rall_deviation * (1.0 + sibling_ratio ** (-3.0 / 2.0))) ** (
            2.0 / 3.0
        )
    return 1.0
...
def terminal_diameters(neurite, method="mean", threshold=1.0, seq=None, bounds=None):
    ...
    if method == "mean":
        term_diam = [
            np.mean(get_diameters(t))
            for t in nm.core.Tree.ileaf(next(iter_sections(neurite)))
            if np.mean(get_diameters(t)) < threshold * mean_diameter
        ]
...
def taper(neurite, params=None, seq=None, only_first=False):
    ...
    if only_first:
        ...
        fit = polynomial.polyfit(
            lengths, get_diameters(root_section), fit_order, full=True
        )
        tap = fit[0][-1]
        if -0.1 < tap < 0:  # only consider negative taper here
            return [
                -tap,
            ]
        return []
