...
def plot_fit_distribution_params(model, neurite_types, fig_name='test', ext='.png'):  # noqa, pylint: disable=too-many-locals
    ...
    try:
        ...
        for neurite_type in neurite_types:
            ...
            var_w = np.array([v['num_value']
                              for v in model[neurite_type]['params']['params_data'].values()])

            # prevent large values of a from bad fitting
            var_as[var_as > utils.A_MAX] = utils.A_MAX
            ax1.scatter(tpes_model, var_as, s=var_w, edgecolors=COLORS[neurite_type],
                        facecolors='none')
    ...
    for neurite_type in neurite_types:
        ...
        ax2.scatter(tpes_model, locs, s=var_w, edgecolors=COLORS[neurite_type], facecolors='none')
    ax2.set_ylabel('loc')
    ...
    for neurite_type in neurite_types:
        ...
        ax3.scatter(tpes_model, scales, s=var_w, edgecolors=COLORS[neurite_type], facecolors='none')
    ...
    for neurite_type in neurite_types:
        ...
        ax4.scatter(tpes_model, mins, s=var_w, edgecolors=COLORS[neurite_type], facecolors='none')
    ax4.set_ylabel('min')
    ...
    for neurite_type in neurite_types:
        ...
        ax5.scatter(tpes_model, maxs, s=var_w, edgecolors=COLORS[neurite_type], facecolors='none')
...
def plot_distribution_fit(  # noqa, pylint: disable=too-many-locals,too-many-arguments,too-many-branches,too-many-statements
    ...
    if isinstance(
            # if the fits are done sequantially
            model[neurite_type]['sequential'], str) and \
            model[neurite_type]['sequential'] != 'asymmetry_threshold':
...
def plot_fit_param_boxes(  # pylint: disable=too-many-locals,too-many-arguments,too-many-branches
    ...
    for mtype in model_params:
        for fit in model_params[mtype][model]:
            if fit != 'trunk_diameter':
                ...
                for params in model_params[mtype][model][fit][neurite_type]['params']:
                    params_0 = model_params[mtype][model][fit][neurite_type]['params'][params][0]
                    params_1 = model_params[mtype][model][fit][neurite_type]['params'][params][1]
    ...
    for mtype in model_params:
        ...
        for _ in fit_gen:
            ...
            bo_gen = (bo for bo in model_params[mtype][model][fit][neurite_type]['params_data']
                      if bo in bos)
            for branch_order in bo_gen:
                bo_list = model_params[mtype][model][fit][neurite_type]['params_data'][branch_order]
...
def draw_axis(obj, mode='2d', axis=None, **kwargs):
    ...
    if mode in ('2d', 'dendrogram'):
        if not axis:
            fig, axis = common.get_figure()
        else:
            fig, _ = common.get_figure()

    else:
        fig, axis = common.get_figure(params={'projection': '3d'})
    ...
    plotter(axis, obj, **kwargs)

    if mode != 'dendrogram':
        common.plot_style(fig=fig, ax=axis, **kwargs)
...
def plot_diameter_diff(  # pylint: disable=too-many-locals,too-many-arguments
    ...
    draw_axis(neuron_orig, axis=axs[0, 0], mode=mode)
    axs[0, 0].set_title('Original neuron')

    draw_axis(neuron_new, axis=axs[0, 1], mode=mode)
    ...
    draw_axis(neuron_diff_pos, axis=axs[1, 0], mode=mode)
    axs[1, 0].set_title('Positive diameter differences')
    draw_axis(neuron_diff_neg, axis=axs[1, 1], mode=mode)
...
def _create_data(feature1, feature2, original_cells, diametrized_cells, step_size, neurite_types):  # noqa, pylint: disable=too-many-locals,too-many-arguments

    def feature_data(cell, neurite_type):
        nm_neurite_type = STR_TO_TYPES[neurite_type]
        return [neurom.get(feat, cell, neurite_type=nm_neurite_type)
...
def plot_cumulative_distribution(  # noqa, pylint: disable=too-many-statements,too-many-locals,too-many-arguments
    ...
    for (bin_centers, stats1, stats2) in data_generator:
        ...
        if len(stats1) > 1:  # don't plot std if there is only one curve
            ...
            # axes[0].fill_between(bin_centers, means - sdevs, means + sdevs, color=color, alpha=.2)
        ...
        if len(stats2) > 1:
            ...
            # axes[0].fill_between(bin_centers, means - sdevs, means + sdevs, color=color, alpha=.5)
            axes[0].plot(bin_centers, means - sdevs, c=color, linestyle='--', lw=3)
            axes[0].plot(bin_centers, means + sdevs, c=color, linestyle='--', lw=3)
